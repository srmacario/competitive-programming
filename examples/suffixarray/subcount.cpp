...
int main(){
    ...
    sz = suf.size()-1;
    //for(int i=1;i<=sz;i++) cout << suf[i] << " " << t.substr(suf[i],sz-suf[i]) << "\n";
    cin >> n;
    while(n--){
        bool ok = false;
        string s;
        cin >> s;
        int szi = s.size(), lb = 0, ub = 0;
        //with sz = suf.size()-1 and disconsidering suf[0], boundaries are:
        int l = 1, r = sz;
        //lower bound
        while(l<r){
            int mid = (l+r)/2;
            //if size of string i is greater than our suffix, we only compare the our prefix of string i with suffix
            if(szi > sz - suf[mid]){
        ...
        int ans;
        //db(lb), db(ub);
        //corner cases in boundaries
        if(szi > sz - suf[lb]) lb++;
        else if(t.substr(suf[lb],szi) != s) lb++;

        if(szi > sz - suf[ub]) ub--;
        else if(t.substr(suf[ub],szi) != s) ub--;
        
        if(ub < lb) ans = 0;
        else if(ub == lb){
            if(szi > sz - suf[lb]) ans = 0;
            else{
                if(t.substr(suf[lb],szi) == s) ans = 1;
                else ans = 0;
            }
        }
        else ans = ub-lb+1;
