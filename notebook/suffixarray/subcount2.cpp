// better version for substring counting
#include <bits/stdc++.h>
...
int main(){
    ...
    while(n--){
        ...
        cin >> s;
        int szi = s.size();
        int l = 1, r = sz, cur = 0, pos = 0;
        while(ok){
            int lb,ub, oldl = l, oldr = r;
            //lower bound
            while(l<r){
                int m = (l+r)/2;
                if(t[suf[m]+pos] < s[cur]) l = m+1;
                else r = m;
            }
            lb = l;
            l = oldl, r = oldr;
