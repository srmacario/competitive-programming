...
vi suffix(string s){
    ...
    k = 0;
    for(int i=0;i<n-1;i++){
        //c[i] is position of suffix i into the suffix array;
        int pi = c[i];
        //if(pi == 0) continue;
        //former suffix;
        int j = p[pi-1];
        //increase k whenever possible to calculate lcp from this two suffixes
        while((i+k < n) and (j+k<n) and s[i+k] == s[j+k]) k++;
        lcp[pi] = k;
        //for the next suffix the k-1 first characters are already pre calc
        k = max(k-1,0);
    }
    return p;
}
...
int main(){
    ...
    vi suf = suffix(t);
    pii ans = {0,0};
    //for(int i=0;i<n;i++) cout << ((suf[i]/sz) and 1) << " " << lcp[i] << " " << t.substr(suf[i],n-suf[i]) << "\n";
    for(int i=2;i<n;i++){
        if((suf[i]/sz) ^ (suf[i-1]/sz)){
            if(lcp[i] == ans.st) ans.nd = min(ans.nd,suf[i]);
            if(lcp[i] > ans.st) ans = {lcp[i],suf[i]};
        }
    }
    cout << t.substr(ans.nd,ans.st) << "\n";
    return 0;
}

 and m[i][j][pos] == m[i-1][j-1][nxt]+1