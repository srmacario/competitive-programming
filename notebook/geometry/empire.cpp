...
int main(){
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < voronoi[i].size(); j++){
            PT p = voronoi[i][j];
            if(le((!p), r * r) and le(ans, (!(p - pts[i])))) ans = !(p - pts[i]);
            ...
            for(auto it: inters){
                if(is_in_voronoi(i, it, pts) and le(ans, !(pts[i] - it))) ans = !(pts[i] - it);
            }
            //db(p _ ans);
        }
