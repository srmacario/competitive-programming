...
class PowerSupply{
    public:
        int maxProfit(std::vector<int>& x, std::vector<int>& y, int D){
            int ans = 0, tmp = 0;
            //nao da pra pegar assim
            int n = 10;
            //db(n);
            //line x
            for(int i = 0; i < n; i++){
            ...
            //line y = x
            for(int i = 0; i < n; i++){
                long long dist = (long long)1 * (x[i] + y[i]) * (x[i] + y[i]);
                if(dist <= (long long)2 * D * D) tmp++;
            }
            if(tmp > ans) ans = tmp;
            tmp = 0;
...
vector<int> x = {0,3,0,0,0,4};
vector<int> y = {1,2,6,-4,1,4};
int D = 1;
