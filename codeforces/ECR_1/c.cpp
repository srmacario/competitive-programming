...
#define db(x) cerr << setprecision(16) << fixed << #x << " == " << x << endl
...
int main(){
    ...
    for(int i = 0; i < n; i ++){
        pts[i].st = (i + 1);
        cin >> pts[i].nd.x >> pts[i].nd.y;
    }
    pair <pair<point, point>, pair<int, int>> ans = {{{-LINF, -LINF}, {-LINF, -LINF}}, {0, 0}};
    sort(pts, pts + n, cmp);
    for(int i = 0; i < n; i ++){
        point a = pts[i].nd;
        point b = pts[(i + 1)%n].nd;
        ld angle;
        pair <pair<point, point>, pair<int, int>> tmp = {{a, b}, {pts[i].st, pts[(i + 1)%n].st}};
        if(!i) ans = tmp;
        else{
            if(angleLess(a, b, ans.st.st, ans.st.nd)) ans = tmp;
        }
    }
    // db(ans.st);
