...
int search_left(point p, vector<point> tmp_pts){
    ...
    while(l < r){
        int mid = (l + r)/2;
        //db(p _ a _ tmp_pts[mid] _ direction(p, a, tmp_pts[mid]));
...
int search_right(point p, vector<point> tmp_pts){
    ...
    while(l < r){
        int mid = (l + r + 1)/2;
        //db(p _ b _ tmp_pts[mid] _ direction(p, b, tmp_pts[mid]));
...
int main(){
    ...
    for(auto e : edges){
        ...
        for(int lft = 0; lft < l; lft++){
            ...
            //db(r + 1 _ n _ l _ r _ idx);
            ans += (n - idx);
            if(idx != n){
                vector<point> tmp_pts;                
                a = pts[l], b = pts[r];
                for(int i = idx; i < n; i++) /*db(a _ b _ pts[lft] _ pts[i]),*/ tmp_pts.push_back(pts[i]);
                sort(tmp_pts.begin(), tmp_pts.end(), cmp2);
                int idx_left = search_left(pts[lft], tmp_pts);
                int idx_right = search_right(pts[lft], tmp_pts);
                //db(idx_left _ idx_right);
                if(idx_left >= (int)tmp_pts.size()) continue;
                if(idx_right < 0) continue;
