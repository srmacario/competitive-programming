//using two pointers / rotating calipers
#include <bits/stdc++.h>
...
int main(){
    ...
    while(true){
        ...
        monotone_hull(pts);
        n = pts.size();
        //if theres redundant points, no if needed
        /*if(n < 3){
            cout << "Case " << k << ": 0.00\n";
            continue;
        }*/
        ...
        for(int l = 0; l < n; l++){
            int r = l + 1, p = l + 2;
            ld tmp = distance_point_line(pts[p], pts[l], pts[r]);
            while((p + 1 < l + n) and le(distance_point_line(pts[p], pts[l], pts[r]), distance_point_line(pts[p + 1], pts[l], pts[r]))){
                tmp = distance_point_line(pts[++p], pts[l], pts[r]);
            }
            ans = min(ans, tmp);
        }
